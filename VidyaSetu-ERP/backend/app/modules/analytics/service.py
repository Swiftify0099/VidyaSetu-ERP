...
class AnalyticsService:
    ...
    def master_dashboard(db: Session, academic_year_id: int = 1) -> MasterDashboard:
        ...
        # Attendance today (approximate — based on MonthlyAttendanceSummary if available)
        today_att_pct = 0.0
        try:
            ...
            today_att_pct = round((present / total * 100) if total else 0, 1)
        except Exception:
            pass
        ...
        try:
            ...
            # Total demanded = sum of all StudentFeeRecord amounts
            demanded = float(db.scalar(select(func.sum(StudentFeeRecord.amount)).where(
                StudentFeeRecord.is_deleted == False,
                StudentFeeRecord.academic_year_id == academic_year_id,
            )) or 0)
            # Total collected = sum of all FeePayment amounts
            ...
            fee_pct = round(collected / demanded * 100, 1) if demanded else 0

            # Monthly revenue for chart
            for month in range(1, 13):
                m_collected = db.scalar(select(func.sum(FeePayment.amount)).where(
                    FeePayment.is_deleted == False,
                    FeePayment.academic_year_id == academic_year_id,
    ...
    def student_strength(db: Session, academic_year_id: int = 1) -> StudentStrengthReport:
        ...
        for r in rows:
            ...
            by_std.append({"standard": r[0], "boys": b, "girls": g, "total": int(r[1] or 0)})
    ...
    def attendance_summary(db: Session, academic_year_id: int = 1) -> AttendanceReport:
        try:
            ...
            overall_pct = round(total_p / total_w * 100, 1) if total_w else 0

            defaulters = [s for s in summaries if total_w and (s.present_days / total_w * 100) < 75]

            by_standard: dict[str, list] = {}
            for s in summaries:
                by_standard.setdefault(s.standard, []).append(s)

            by_std_list = []
            for std, ss in sorted(by_standard.items()):
                p = sum(x.present_days for x in ss)
                w = sum(x.working_days for x in ss)
                by_std_list.append({"standard": std, "present_pct": round(p/w*100, 1) if w else 0})
    ...
    def fee_report(db: Session, academic_year_id: int = 1) -> FeeReport:
        try:
            ...
            pending = max(0, demanded - collected)
            pct = round(collected / demanded * 100, 1) if demanded else 0
